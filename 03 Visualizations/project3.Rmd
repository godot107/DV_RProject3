...
After Joining birth rate with populations of countries, we attempted to group the countries into regions. Then we averaged the birth rates of those regions to see if there is a relationship between birth rates. 
...
We combined literacy rates and a country's GDP to see if there's a correlation.
```{r}
# Loads the datawrangling and visualizations with examples of inner join, outer join, and combining data sets
source("../02 Data Wrangling/combine.R", echo = TRUE)
```

In the below histogram we compared the GDP of countries in dollars.
```{r}
